...
class Pipelines(BaseKnowledge):
    ...
    def __init__(self, leader, args=None, logger=None, group=None, fleet=None, **kwargs):
        ...
        if group is not None or fleet is not None:
            ...
            self.endpoint = f"m/{self.group}/pipelines"
    ...
    def update(self, item=None):
        ...
        try:
            if "groups" in item and isinstance(item["groups"], list):
                item["groups"] = {}
            if "conf" in item and "groups" in item["conf"] and isinstance(item["conf"]["groups"], list):
                item["conf"]["groups"] = {}
    ...
    def simulate(self, item=None):
        if item is None:
            item = {}
        action = f"simulate_{self.obj_type}"
        changes = {"id": item["id"] if "id" in item else "Unknown",
                   "previous": {"status": "does_not_exist", "data": {}},
                   "current": {"status": "import_data", "data": item}}
        self._log("debug", action=action, item=item)
        try:
            self._log("info", action=action,
                      item_type=self.obj_type,
                      item_id=item["id"],
